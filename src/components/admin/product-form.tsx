  ...
  const imgs =
    p.images && p.images.length > 0
      ...
      : p.image
        ? [p.image]
        : [];
  const packItems: PackItem[] = p.packItems?.length
    ? [...p.packItems]
    : [];
  return {
    nameFr: p.nameFr,
...
export function ProductForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      packItems: draft.isPack ? draft.packItems : [],
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          ...
          {draft.isPack ? (
            <p className="mt-1.5 text-xs text-stone">{t("formPackCategoryHint")}</p>
          ) : null}
        </div>
        <div>
          <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-stone">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3">
                      ...
                      <input
                        value={
                          it.image.startsWith("data:") ? "" : it.image
                        }
                        onChange={(e) =>
                          updatePackItem(index, {
                            ...it,
